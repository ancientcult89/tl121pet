...
namespace tl121pet.Controllers
{
    public class MeetingController : Controller
    {
        ...
        {
            return View("MeetingTypeEditor", new MeetingTypeVM() { SelectedMeetingType = _dataContext.MeetingType.Find(id) ?? new MeetingType(), Mode = FormMode.Edit });
        }

        [HttpPost]
        public IActionResult MeetingTypeEdit([FromForm] MeetingTypeVM meetingTypeVM)
        {
            if (ModelState.IsValid)
            {
                _meetingRepository.UpdateMeetingType(meetingTypeVM.SelectedMeetingType);
        ...
        {
            return View("MeetingTypeEditor", new MeetingTypeVM() { SelectedMeetingType = _dataContext.MeetingType.Find(id) ?? new MeetingType(), Mode = FormMode.Details });
        }

        public IActionResult MeetingTypeCreate()
        {
            return View("MeetingTypeEditor", new MeetingTypeVM() { SelectedMeetingType = new MeetingType(), Mode = FormMode.Edit });
        }

        [HttpPost]
        public IActionResult MeetingTypeCreate([FromForm] MeetingTypeVM meetingTypeVM)
        {
            if (ModelState.IsValid)
            {
                _meetingRepository.UpdateMeetingType(meetingTypeVM.SelectedMeetingType);
